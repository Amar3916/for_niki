                    ...
                    <ResultsDisplay 
                      ...
                      onDownload={(format) => {
                        ...
                        if (format === 'json') {
                          ...
                          filename = `search-results-${new Date().toISOString().slice(0, 10)}.json`;
                          downloadFile(content, filename, 'application/json');
                        } else if (format === 'csv') {
                          // Create CSV content
                          const headers = ['id', 'title', 'description', 'url', 'price', 'rating'];
                          const rows = message.results.items.map(item => [
                            item.id,
                            item.title,
                            item.description || '',
                            item.url || '',
                          ...
                          filename = `search-results-${new Date().toISOString().slice(0, 10)}.csv`;
